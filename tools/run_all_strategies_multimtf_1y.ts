...
async function main() {
  console.log(`[run] 開始執行 ${SYMBOL} 一年期 MTF 全策略回測（4H 趨勢 + 15m 進場）...`);
  console.log(`[run] 目標 K 線數：15m=${COUNT_15M}, 4H=${COUNT_4H}`);
  ...
  const jsonPath = path.join(OUT_DIR, `backtest_all_strategies_multimtf_1y_${SYMBOL}_${stamp}.json`);
  const csvPath = path.join(OUT_DIR, `backtest_all_strategies_multimtf_1y_${SYMBOL}_${stamp}.csv`);
  const mdPath = path.join(OUT_DIR, `backtest_all_strategies_multimtf_1y_${SYMBOL}_${stamp}.md`);
...
main().catch((error) => {
  console.error("[error] 全策略一年期 MTF 回測失敗");
