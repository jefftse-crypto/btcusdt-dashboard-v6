...
function detectCrtModel1(snapshot: CryptoSnapshot | null | undefined, currentPrice: number | null | undefined) {
  ...
  const chochLevel = recentChoch.level ?? recentChoch.price ?? currentPrice;
  ...
  if (isBull && swingLow) {
    ...
    return {
      direction: "bullish",
      trigger: `CHoCH 看漲確認 @ ${chochLevel.toFixed(2)}`,
      stab: `假突破舊低 ${swingLow.toFixed(2)}（誘空蠟燭）`,
      entry: `等待實體收盤突破 CHoCH 水位（${chochLevel.toFixed(2)}）後做多`,
  ...
  if (!isBull && swingHigh) {
    ...
    return {
      direction: "bearish",
      trigger: `CHoCH 看跌確認 @ ${chochLevel.toFixed(2)}`,
      stab: `假突破舊高 ${swingHigh.toFixed(2)}（誘多蠟燭）`,
      entry: `等待實體收盤跌破 CHoCH 水位（${chochLevel.toFixed(2)}）後做空`,
        ...
        <div className="p-3 space-y-1.5">
          {[
            ...
            },
            {
              ...
              note: (() => { const choch = (smc?.bos_choch ?? []).find(b => b.type === "CHoCH"); const level = choch?.level ?? choch?.price; return typeof level === "number" ? level.toFixed(2) : "未偵測到"; })(),
            },
            {
