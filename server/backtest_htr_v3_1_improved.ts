...
function buildSignalsForSymbol(symbol:string, c:Candle[], btcData:Candle[], cfg:Cfg): Signal[] {
  ...
  for(let i=240;i<c.length-2;i++){
    ...
    // BTC Filter
    if (cfg.btcFilter) {
        const btcTrendUp = btc4[btcIdx4].close > btc_e4_50[btcIdx4];
        if (!btcTrendUp) continue;
    }
    ...
    if(trend4Up && state1Up){
      ...
      if(entry>high12 && c[i-1].close<=high12){score+=1.05; notes.push('突破短結構高點');}
      ...
      if(risk>0 && score>=cfg.score && lev>0 && riskPct>=cfg.minRiskPct && riskPct<=cfg.maxRiskPct) sigs.push({symbol,i,time:c[i].time,dir:'long',score,entry,stop,riskPct,leverage:lev,notes,targets:[1.1,2.1,3.0],parts:[0.45,0.35,0.20],regime:'trend_up'});
    }
  }
  return sigs;
}
...
function simulateOne(c:Candle[], s:Signal, cfg:Cfg, riskPerTrade=0.0035): Trade {
  ...
  for(let j=s.i+1;j<=Math.min(c.length-1,s.i+96);j++){
    ...
    for(let k=0;k<levels.length;k++) if(!filled[k]){ const hit=s.dir==='long'?high>=levels[k]:low<=levels[k]; if(hit){ const part=s.parts[k]; r += part*s.targets[k]; remaining-=part; filled[k]=true; 
        if(k===0 && cfg.useBreakeven) stop=s.entry + (s.dir==='long'?0.05*risk:-0.05*risk);
...
async function main(){
  const allSymbols=['BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','XRPUSDT','DOGEUSDT','LINKUSDT','AVAXUSDT']; const data:Record<string,Candle[]>={};
  ...
  const configs:Cfg[]=[
    {name:'v3_baseline_pool8', symbols:pool8, score:7.0, minRr:2.0, minRiskPct:0.0016, maxRiskPct:0.009, session:'asia_eu_us', cooldownHours:0, useBreakeven:false, btcFilter:false},
    {name:'v3_1_improved_pool8', symbols:pool8, score:7.2, minRr:2.0, minRiskPct:0.0016, maxRiskPct:0.009, session:'asia_eu_us', cooldownHours:0, useBreakeven:true, btcFilter:true},
  ];
  const summaries:any[]=[];
  ...
  fs.writeFileSync(path.join(OUT_DIR, 'v3_1_improvement_summary.json'), JSON.stringify(summaries, null, 2));
}
