...
export function detectPaPatternsWithLevels(
  ...
  for (let i = candles.length - lookback; i < candles.length - 1; i++) {
    ...
    // ── Opus 4.6 P1 改良：將 EMA / 量能計算移出 pattern 迢圈（避免重複計算）──
    const recentCandles = candles.slice(Math.max(0, i - 20), i + 1);
    const ema5 = recentCandles.slice(-5).reduce((s, cc) => s + cc.close, 0) / Math.min(5, recentCandles.length);
    const ema20 = recentCandles.reduce((s, cc) => s + cc.close, 0) / Math.max(1, recentCandles.length);
    const shortTermTrend = ema5 > ema20 * 1.001 ? "bullish" : ema5 < ema20 * 0.999 ? "bearish" : "ranging";
    const avgVol = candles.slice(Math.max(0, i - 20), i).reduce((s, cc) => s + cc.volume, 0) / 20;
...
export function detectSmcConfirmationSetups(
  ...
  const validatedSetups = setups.filter(setup => {
    ...
    if (fvgIdx == null || obIdx == null || fvgIdx === -1 || obIdx === -1) return true; // invalidated 記錄已處理
