/**
 * Kelly Fraction Calibration Service v2.1
 * 
 * v5.1 改良點（來自深度分析報告）：
...
export function calibrateKelly(input: KellyCalibrationInput): KellyCalibrationResult {
  ...
  const dynamicKellyFactor = calcDynamicKellyFactor(recentPnl ?? [], recentVolatility);
  ...
  const adjustedMaxPositionPct = maxPositionPct * nEffFactor * correlationPenalty;
