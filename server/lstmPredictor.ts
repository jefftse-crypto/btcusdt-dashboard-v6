...
function extractFeatures(candles: Candle[]): number[][] {
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    const stDirection = stArr[i]?.direction;
    const stDir = stDirection === 1 ? 1 : -1;
...
export async function predictLstm(
  ...
  if (needRetrain) {
    ...
    cache = modelCache.get(cacheKey) ?? null;
    if (!cache) throw new Error(`LSTM 模型訓練後未能載入快取：${cacheKey}`);
  } else {
    if (!cache) throw new Error(`LSTM 模型快取不存在：${cacheKey}`);
    // 更新最新特徵（使用最新 K 線）
    const allFeatures = extractFeatures(candles);
