...
async function main(){
  ...
  fs.writeFileSync(path.join(OUT_DIR,'htr_v3_multisymbol_backtest_1y.json'), JSON.stringify(output,null,2));
  ...
  const md=`# HTR V3 多币种趋势策略一年回测\n\n数据源：Binance Data Vision USDT-M Futures 15m 月度 K 线。区间：${output.first} 至 ${output.last}。V3 使用 4H 定方向、1H 确认交易状态、15m 入场触发，并在多币种同一时间出现信号时选择最高分；每个 UTC 日期最多一笔，账户风险固定 ${(riskPerTrade*100).toFixed(2)}%。\n\n| 配置 | 币种数 | 原始信号 | 实际交易 | 天/笔 | 胜率 | PF | 期望值 | 账户收益 | 最大回撤 |\n|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n${rows}\n\n## 当前最佳\n\n综合频率、PF、收益、胜率与回撤后，当前最佳为 **${best.config}**：一年实际交易 **${best.trades} 笔**，约 **${best.daysPerTrade.toFixed(2)} 天一笔**，胜率 **${best.winRate.toFixed(2)}%**，PF **${best.profitFactor.toFixed(2)}**，账户结果 **${best.accountReturnPct.toFixed(2)}%**，最大回撤 **${best.maxDrawdownPct.toFixed(2)}%**。\n`;
  fs.writeFileSync(path.join(OUT_DIR,'htr_v3_multisymbol_backtest_1y.md'), md);
  console.log(md);
}
