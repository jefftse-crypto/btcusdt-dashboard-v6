...
// ─────────────────────────────────────────────────────────────────────────────
// [B1] Log-odds 去相關化貝葉斯聚合（v2.0 新增）
// GPT-5.4 審查發現：原乘法聚合將相關維度（EMA/MACD/布林）當作獨立事件，導致訊號重複計算
// 此函數使用 log-odds 去相關化，模擬「結構相關性矩陣」的效果
// ─────────────────────────────────────────────────────────────────────────────

export interface DecorrelatedSignal {
  name: string;
  /** 原始概率 0-1 */
  probability: number;
  /** 與其他維度的相關性群組 ID（相同群組的維度會被去相關化） */
...
/**
 * [B1] Log-odds 去相關化貝葉斯聚合
 *
 * 原理：
 * 1. 將每個維度的概率轉換為 log-odds： logit(p) = log(p/(1-p))
 * 2. 同一相關性群組內，只取最強的信號（避免重複計算）
 * 3. 將去相關化後的 log-odds 加權累加
 * 4. 轉回概率： sigmoid(sum_logodds)
 *
 * 典型相關性群組：
 * - "trend"：EMA、MACD、ADX（都是趨勢追蹤指標）
 * - "momentum"：RSI、KD、CCI（都是動量指標）
 * - "volatility"：布林通道、ATR（都是波動率指標）
 * - "structure"：SMC、續論、BOS/CHoCH（結構分析）
 * - "volume"：OBV、成交量比率（量能指標）
 *
 * @param signals 訊號陣列（包含相關性群組資訊）
 * @param priorProbability 先驗概率（預設 0.5 = 無資訊先驗）
 * @returns 去相關化後的後驗概率和詳細資訊
 */
export function aggregateDecorrelatedSignals(
  signals: DecorrelatedSignal[],
  priorProbability = 0.5,
): {
  posteriorProbability: number; // 後驗概率 0-1
  posteriorScore: number;       // 後驗分數 0-100
  effectiveSignalCount: number; // 有效信號數（去相關化後）
  groupDetails: { group: string; selectedSignal: string; logOddsContribution: number }[];
} {
  if (signals.length === 0) {
    return { posteriorProbability: priorProbability, posteriorScore: priorProbability * 100, effectiveSignalCount: 0, groupDetails: [] };
  }

  // Step 1: 將每個維度轉換為 log-odds
  const logit = (p: number) => Math.log(Math.max(0.001, Math.min(0.999, p)) / (1 - Math.max(0.001, Math.min(0.999, p))));
  const sigmoid = (x: number) => 1 / (1 + Math.exp(-x));

  // Step 2: 按相關性群組分組，同群組只取最強的信號
  const groups = new Map<string, DecorrelatedSignal[]>();
  for (const sig of signals) {
    const g = groups.get(sig.correlationGroup) ?? [];
    g.push(sig);
    groups.set(sig.correlationGroup, g);
  }

  const groupDetails: { group: string; selectedSignal: string; logOddsContribution: number }[] = [];
  let sumLogOdds = logit(priorProbability); // 從先驗 log-odds 開始
  let effectiveSignalCount = 0;

  for (const [groupName, groupSignals] of Array.from(groups.entries())) {
    // 同群組只取權重最高的信號（避免相關性重複）
    const best = groupSignals.sort((a: DecorrelatedSignal, b: DecorrelatedSignal) => b.weight - a.weight)[0];
    const logOddsSignal = logit(best.probability);
    const logOddsPrior = logit(priorProbability);
    // 去相關化貢獻：只加入訊號相對於先驗的額外資訊
    const contribution = (logOddsSignal - logOddsPrior) * best.weight;
    sumLogOdds += contribution;
    effectiveSignalCount++;
    groupDetails.push({
      group: groupName,
      selectedSignal: best.name,
      logOddsContribution: Math.round(contribution * 100) / 100,
    });
  }

  // Step 3: 轉回概率
  const posteriorProbability = sigmoid(sumLogOdds);
  const posteriorScore = Math.round(posteriorProbability * 100);

  return { posteriorProbability, posteriorScore, effectiveSignalCount, groupDetails };
}
