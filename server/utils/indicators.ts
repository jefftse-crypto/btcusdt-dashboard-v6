...
export function calcChanSimple(candles: Candle[]) {
  ...
  // [修復 v3.1] 纏論筆最小間距：使用原始 K 棒索引差（而非合並後索引差）
  // Bug 根因：合並後 merged 陣列大幅縮小（例如 500→100），merged.idx 差通常只有 2-4
  // 導致所有筆都因「間距不足」被過濾，biCount 永遠為 0
  // 修復：改用 mergedToOriginalEnd[idx] 取得原始索引，確保間距判斷基於真實 K 棒數量
  const MIN_BI_GAP_ORIG = 5; // 原始 K 棒數間距
...
/**
 * Calculate estimated CVD (Cumulative Volume Delta) from OHLCV candles.
 * Positive delta is assigned to bullish candles, negative delta to bearish candles,
 * and doji candles contribute zero. The returned series is index-aligned with input candles.
 */
export function calcCVD(candles: Array<{ open: number; close: number; volume: number }>): number[] {
  const series: number[] = [];
  let cumulative = 0;
...
export interface SupertrendPoint {
  value: number;
  direction: 1 | -1; // 1 = bullish (price above), -1 = bearish (price below)
}
...
export function calcSupertrend(
  ...
  const n = candles.length;
  const result: SupertrendPoint[] = new Array(n).fill({ value: NaN, direction: 1 });
  ...
  return candles.map((_, i) => ({
    ...
    direction: direction[i] as 1 | -1,
  }));
}

export function calcSupertrendLast(candles: Candle[], period = 10, multiplier = 3): SupertrendPoint {
...
export interface IchimokuPoint {
  tenkan: number;   // 轉換線 (9)
  kijun: number;    // 基準線 (26)
  senkou_a: number; // 先行帶 A (26 periods ahead)
  senkou_b: number; // 先行帶 B (52 periods ahead)
  chikou: number;   // 遲行帶 (26 periods back)
}

function highestHigh(candles: Candle[], from: number, period: number): number {
  let h = -Infinity;
  for (let i = Math.max(0, from - period + 1); i <= from; i++) h = Math.max(h, candles[i].high);
  return h;
}
function lowestLow(candles: Candle[], from: number, period: number): number {
  let l = Infinity;
...
export function calcIchimokuLast(candles: Candle[]): IchimokuPoint {
...
export function calcPivotPoints(candles: Candle[]): PivotPoints {
  // Use previous completed session (last closed candle)
...
// ─────────────────────────────────────────────────────────────────────────────
export function calcDemaArr(data: number[], period: number): number[] {
...
export function calcTemaArr(data: number[], period: number): number[] {
  const ema1 = calcEmaArr(data, period);
  const ema2 = calcEmaArr(ema1, period);
...
export function calcDonchianArr(candles: Candle[], period = 20): DonchianPoint[] {
  return candles.map((_, i) => {
...
export function calcDonchianLast(candles: Candle[], period = 20): DonchianPoint {
...
// ─────────────────────────────────────────────────────────────────────────────
export function calcCmfArr(candles: Candle[], period = 20): number[] {
  return candles.map((_, i) => {
...
export function calcCmfLast(candles: Candle[], period = 20): number {
...
// ─────────────────────────────────────────────────────────────────────────────
export function calcHmaArr(data: number[], period: number): number[] {
...
export function detectRsiDivergence(candles: Candle[], rsiArr: number[], lookback = 30): DivergenceResult {
  const n = candles.length;
  ...
  // Find recent swing lows (price) and corresponding RSI
