/**
 * 參數搜索 - 組 B：布林帶、MACD、ICT流動性、VWAP回歸
 */
import fs from "fs";
...
const configs = [
  { strategy: "bollinger", label: "布林帶" },
  { strategy: "macd", label: "MACD" },
  { strategy: "liquidity_sweep", label: "ICT流動性" },
  { strategy: "vwap_reversion", label: "VWAP回歸" },
];
...
fs.writeFileSync(path.join(BASE, "search_groupB.json"), JSON.stringify(results, null, 2));
console.log("\n✅ 組B完成");
