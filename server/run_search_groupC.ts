/**
 * 參數搜索 - 組 C：複合策略、CannonBall、HWR-A、HWR-B、HWR-C
 */
import fs from "fs";
...
const configs = [
  { strategy: "composite", label: "複合策略" },
  { strategy: "cannonball", label: "CannonBall" },
  { strategy: "hwr_model_a", label: "HWR-A" },
  { strategy: "hwr_model_b", label: "HWR-B" },
  { strategy: "hwr_model_c", label: "HWR-C" },
];
...
fs.writeFileSync(path.join(BASE, "search_groupC.json"), JSON.stringify(results, null, 2));
console.log("\n✅ 組C完成");
