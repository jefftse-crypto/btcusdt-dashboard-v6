...
function buildSmc(candles: Candle[], ind: IndicatorData): CryptoSnapshot["smc"] {
  ...
  const liquidityLevels = [
    ...highs.slice(-5).map(h => ({ price: h.price, type: "buy_side" as const, swept: price > h.price })),
    ...lows.slice(-5).map(l => ({ price: l.price, type: "sell_side" as const, swept: price < l.price })),
