...
async function main(){
  ...
  const tpvBest2=['BNBUSDT','XRPUSDT'];
  const tpvBest3=['BTCUSDT','BNBUSDT','XRPUSDT'];
  const tpvBest4=['BTCUSDT','BNBUSDT','XRPUSDT','AVAXUSDT'];
  const configs:Cfg[]=[
    ...
    {name:'v4_tpv_best2_quality', symbols:tpvBest2, score:8.2, minRr:2.4, minRiskPct:0.0014, maxRiskPct:0.010, session:'asia_eu_us', cooldownHours:0},
    {name:'v4_tpv_best2_elite', symbols:tpvBest2, score:9.0, minRr:2.6, minRiskPct:0.0014, maxRiskPct:0.009, session:'eu_us', cooldownHours:4},
    {name:'v4_tpv_best3_quality', symbols:tpvBest3, score:8.2, minRr:2.4, minRiskPct:0.0014, maxRiskPct:0.010, session:'asia_eu_us', cooldownHours:0},
    {name:'v4_tpv_best3_elite', symbols:tpvBest3, score:9.0, minRr:2.6, minRiskPct:0.0014, maxRiskPct:0.009, session:'eu_us', cooldownHours:4},
    {name:'v4_tpv_best4_quality', symbols:tpvBest4, score:8.2, minRr:2.4, minRiskPct:0.0014, maxRiskPct:0.010, session:'asia_eu_us', cooldownHours:0},
  ];
  ...
  fs.writeFileSync(path.join(OUT_DIR,'htr_v4_tpv_multisymbol_backtest_1y.json'), JSON.stringify(output,null,2));
  ...
  const md=`# HTR V4 TPV 启发式多币种策略一年回测\n\n数据源：Binance Data Vision USDT-M Futures 15m 月度 K 线。区间：${output.first} 至 ${output.last}。V4 TPV 使用 4H 定方向、1H 确认交易状态、15m 第三触点与收线验证触发，并在多币种同一时间出现信号时选择最高分；每个 UTC 日期最多一笔，账户风险固定 ${(riskPerTrade*100).toFixed(2)}%。\n\n| 配置 | 币种数 | 原始信号 | 实际交易 | 天/笔 | 胜率 | PF | 期望值 | 账户收益 | 最大回撤 |\n|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n${rows}\n\n## 当前最佳\n\n综合频率、PF、收益、胜率与回撤后，当前 TPV 最佳为 **${best.config}**：一年实际交易 **${best.trades} 笔**，约 **${best.daysPerTrade.toFixed(2)} 天一笔**，胜率 **${best.winRate.toFixed(2)}%**，PF **${best.profitFactor.toFixed(2)}**，账户结果 **${best.accountReturnPct.toFixed(2)}%**，最大回撤 **${best.maxDrawdownPct.toFixed(2)}%**。\n`;
  fs.writeFileSync(path.join(OUT_DIR,'htr_v4_tpv_multisymbol_backtest_1y.md'), md);
  console.log(md);
}
